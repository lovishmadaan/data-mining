...
void insert(vector<int> &set, Node *root) {
    ...
    while(i < setSize && M[set[i]] * 100 >= x * T) {
...
vector<vector<int> > generate_fpsets(vector<vector<int> > &lol) {
    ...
    // }
    // cout<<endl<<endl;
    int lolSize = lol.size();
    if(lolSize==0)return vector<vector<int> >();
    int sum = 0;
    ...
    if(sum * 100 >= x * T)
    {
        ...
        for(int i=0;i<iter;i++)
        {
            ...
            // printsets(tempres);
            int tempresSize = tempres.size();
            for(int k = 0;k<tempresSize;k++)
            {
                result.push_back(tempres[k]);
            }

            for(int j=0;j<lolSize;j++)
            {
                lol[j].pop_back();
            }
        }
